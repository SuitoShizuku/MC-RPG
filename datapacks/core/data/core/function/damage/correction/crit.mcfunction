...
#tellraw @s {score:{name:"@s",objective:"Core.Math.Crit"}}
execute unless score @s Core.Status.CritRate <= @s Core.Math.Crit run return run scoreboard players set @s Core.Math.Crit 0
# 会心
